...
final class ProfilePopupTests: XCTestCase {
    ...
    func testProfileStatsDisplayCorrectData() {
        ...
        let profileStats = ProfileStats(stats: stats)
    ...
    func testProfileHeaderDisplaysCorrectData() {
        let viewModel = MockProfileViewModel()
        let profileHeader = ProfileHeader(viewModel: viewModel)
        XCTAssertEqual(profileHeader.viewModel.userName, "Test User", "ProfileHeader should display the correct user name")
    }
