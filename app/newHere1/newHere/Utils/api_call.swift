...
///   - locationDataManager: A `LocationDataManager` to provide the current location.
///   - completion: A completion handler that returns either `MessageResponse` or `Error`.
func postMessage(user_id: String, text: String, visibility: String, locationDataManager: LocationDataManager, timeoutInterval: TimeInterval = 20.0, completion: @escaping (Result<MessageResponse, Error>) -> Void) {
