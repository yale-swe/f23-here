...
struct MessagesPopup: View {
    ...
    var body: some View {
        ZStack{
            ...
            //FIXME: asset 'cross_campus' not exists!
//            Image("cross_campus")
//                .font(.system(size: 50))
            ScrollView {
                VStack {
                    HStack {
                        ...
                        .padding(.trailing, 20)
                        .accessibilityIdentifier("Xmark Button")
                    }
                    // Loop to create message buttons
                    ForEach(1...5, id: \.self) { count in
                        Button(action: {
                            onClickedMessage(count)
                        }) {
                            HStack {
                                ProfilePicture()
                                Text("Message Title")
    ...
    func onClickedMessage(_ index: Int) {
        //TODO: impl the code
    }
}
/**
...
struct MessagesPopup_Previews: PreviewProvider {
    static var previews: some View {
        MessagesPopup(isPresented: .constant(true))
    }
}
