...
class PostsPopupTests: XCTestCase {
    ...
    func testPostMessageFunctionality() {
        let expectedMessage = "Test Message"
        postsPopup.noteMessage = expectedMessage
        postsPopup.postMessage(user_id: "TestUserID", text: expectedMessage, visibility: "friends", locationDataManager: mockLocationDataManager) { result in
            switch result {
            case .success(let response):
                XCTAssertEqual(mockMessageState.currentMessage?.messageStr, expectedMessage, "Posted message should be updated in messageState")
            case .failure(let error):
                XCTFail("Posting message failed with error: \(error.localizedDescription)")
            }
        }
    }
